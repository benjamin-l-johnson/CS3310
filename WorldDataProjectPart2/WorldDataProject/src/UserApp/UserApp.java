 ...
 *      OUTPUT:  MainData.bin         (handled by MainData class)
...
public class UserApp {
    public static void main(String[] args) throws IOException {
        ...
        MainData mainFile = new MainData(log,false);
    	...
    	while(log.noMoreRequests() != true)
    	{
    		...
    		switch (request[0])
    		{
    			...
    			case "QC":
    				log.displayThis(request[0]+" "+request[1]);
    				mainFile.queryByCode(request[1]);
    				break;
    			
    			case "DC":
    				log.displayThis(request[0]+" "+request[1]);
    				mainFile.deleteByCode(request[1]);
    				break;
